...
```{r results='asis'}
  ...
  # Summary table to latex
    ...
    #stargazer(redistrib[-c(1,2)], type="text"); sink()
    
  # Simple graph:
    sjPlot::plot_frq(redistrib$income_dif, show.values = TRUE, coord.flip = TRUE)
    
```

```{r echo=FALSE}
# Likert graph 
    sjPlot::plot_frq((redistrib[3:7])) # ajustar
    ...
    sjPlot::plot_likert((redistrib[3:7]), 
                        axis.labels   = items,
                        ...
                        grid.range    = 1.40) # expand axis 130% )
    
    
    # sjp.likert((redistrib2[3:7]), 
    #             axis.labels   = items,
    #            legend.labels = labels,
    ...
    # Save
    dev.copy(png,"output/results/likert_red.png",width=800); dev.off()
...
###	################## 	agregar un sumary mas  reducido
...
![IMAGEN PROVISORIA](../output/results/path_red1_res.png)
```{r}
# 4. MODELS A:  MERIT & REDISTRIB ========= =
...
### Hay que remplazar sjp.scatter, por plot_scatter #############################################################
    ### Ademas me dice que el suindice esta fuera de los limites.
    
  #  plot_scatter(pref_agg$gini_net,pref_agg$accum, 
  #  	dot.labels = pref_agg$v5c,
  #  	fit.line = TRUE,
  #  	show.ci = TRUE
	#   	)
  
#    sjp.scatter(pref_agg$gini_net,pref_agg$redist, 
 #   	dot.labels = pref_agg$v5c,
  #  	fit.line = TRUE,
   # 	show.ci = TRUE
	  # 	)
...
# 6.2 Descriptives ---------------------- #####
 ...
 #   names(merit_agg)
  ...
  #  sjp.scatter(merit_agg$gini_net,merit_agg$bmerit, 
  ...
  #  sjp.scatter(merit_agg$gini_net,merit_agg$unmerit, 
   # 	dot.labels = merit_agg$v5c,
    #	fit.line = TRUE,
  #  	show.ci = TRUE
	#   	)    
# 
 #  sjp.scatter(merit_agg$gini_net,merit_agg$need, 
  #  	dot.labels = merit_agg$v5c,
   # 	fit.line = TRUE,
    #	show.ci = TRUE
	   #	)  
...
```{r}
   #sjp.scatter(merit_agg$j_fi_index,merit_agg$need, 
    #	dot.labels = merit_agg$v5c,
  #  	fit.line = TRUE,
   # 	show.ci = TRUE
	  # 	)     
      # Save
     # dev.copy(png,"output/results/gini_fi.png"); dev.off()
```


![Imagen provisoria](../output/results/gini_fi.png)
...
```{r}
  # Sin outliers
    ...
    #sjp.scatter(merit_agg2$gini_net,merit_agg2$unmerit, 
  ...
  # Multilevel models merit
	  ...
	  # Null
	  	mlmerit_0 = lmer(merit ~ 1 + (1 | v5c), data = meritocracy)
			screenreg(mlmerit_0) # de library texreg  
			
			# Graph
		#	sjp.lmer(mlmerit_0, sort.est="(Intercept)")
...
# Models meritocracy and preferences
 ...
 #   names(mer_pref)
  ...
  #    plot_scatter(mer_pref_agg$merit,mer_pref_agg$bmerit, 
   # 	dot.labels = mer_pref_agg$v5c,
    #	fit.line = TRUE,
  #  	show.ci = TRUE
	 #  	)
  #    
  #    plot_scatter(mer_pref_agg$merit,mer_pref_agg$redist, 
  #  	dot.labels = mer_pref_agg$v5c,
   # 	fit.line = TRUE,
    #	show.ci = TRUE
	 #  	)
  ...
  #    dev.copy(png,"loc_merit_pref_int/results/merit_redis.png"); dev.off()
    ...
    #  sjp.scatter(mer_pref_agg$unmerit,mer_pref_agg$redist, 
  #  	dot.labels = mer_pref_agg$v5c,
   # 	fit.line = TRUE,
    #	show.ci = TRUE
	   	#)      
  ...
  #    sjp.scatter(mer_pref_agg$redist,mer_pref_agg$accum, 
  #  	dot.labels = mer_pref_agg$v5c,
  #  	fit.line = TRUE,
  #  	show.ci = TRUE
	#   	)  
