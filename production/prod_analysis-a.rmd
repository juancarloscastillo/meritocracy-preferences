...
In this document "Analysis A" the descriptive and correlational analyzes performed on the database created in data-prep are exposed.

The first section provides descriptive analyzes of the main dependent and independent variables.

In the second section, the factor analyzes are presented from which values are imputed to be used in the multilevel analysis section. The adjustment by country is also exposed.

In the third section, bivariate analyzes are presented: correlations and dispersion graphs. 
...
> Based on the good fit of the models, it is inferred that the proposed dimensionality has valid evidence. There is no great difference between the results of the different estimators. We can also say, depending on the model that includes groups, that this dimensionality works in different countries.
...
>  Based on the good fit of the models, it is inferred that the proposed dimensionality has evidence of validity. There is no great difference between the results of the different estimators. We can also say, depending on the model that includes groups, that this dimensionality works in different countries.
...
**The model does not converge neither in Croatia, Cyprus, Italy nor Latvia**
...
rownames(corMat) <-c(
    ...
    "K. PIB per capita",
    "L. Redistribution",
    "M. Government Effectivinnes"
    )
colnames(corMat) <-c("(A)", "(B)","(C)","(D)","(E)","(F)","(H)","(I)","(J)", "(K)", "(L)", "(M)")
