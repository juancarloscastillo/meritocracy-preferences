...
# Generate new sequential id individuals by country to merge
      issp2009$id <- sequence(tabulate(issp2009
                                     $v5))
    tapply(issp2009$id, issp2009$v5, mean, na.rm=TRUE)
    summary(issp2009$id)
...
```{r}
  ...
  # Checks
# aggregate(redistrib, by=list(redistrib$v5), FUN=mean,na.rm=TRUE) ## Funciona pero no lo guarda en una base ##############
...
```{r}
print(dfSummary(redistrib, valid.col = FALSE, graph.magnif = 1),
...
table(indepvar$degree)
  indepvar$degree[indepvar$degree > 5]=NA
  table(indepvar$degree)

sjPlot::plot_frq(indepvar$degree, coord.flip = TRUE, title = "After")
